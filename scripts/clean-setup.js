...
db.exec('DROP TABLE IF EXISTS lecture_slides');
...
console.log('Creating lecture_slides table...');
db.exec(`
  CREATE TABLE lecture_slides (
    ...
    icon TEXT DEFAULT '📋',
    has_material INTEGER DEFAULT 1,
...
db.prepare(`
  ...
  VALUES (?, ?, ?, ?)
`).run('admin', 'System Administrator', adminPasswordHash, 'admin');

// Create test employee
console.log('Creating test employee...');
const employeePasswordHash = bcrypt.hashSync('test123', 10);
db.prepare(`
  INSERT INTO users (username, name, password_hash, role)
  VALUES (?, ?, ?, ?)
`).run('testuser', 'Test Employee', employeePasswordHash, 'employee');

// Create AML/CFT Test
console.log('\nCreating AML/CFT Compliance Test...');
const testResult = db.prepare(`
  INSERT INTO tests (title, description, time_limit, passing_score, total_marks, question_count)
  VALUES (?, ?, ?, ?, ?, ?)
`).run(
  'AML/CFT Compliance Assessment',
  'Mandatory Anti-Money Laundering and Counter Financing of Terrorism assessment for all Sheba Pay employees. 25 randomly selected questions from a pool of 40. Each question carries 2 marks. Passing score: 70% (35/50 marks).',
  30,
  70,
  50,
  25
...
// Insert all 40 AML questions
console.log('\nInserting 40 AML/CFT questions...');
...
const questions = [
  {
    ...
    correct_option: "B",
    explanation: "Money laundering is the process of hiding the origins of money obtained from illegitimate sources through complex transactions."
  },
  {
    ...
    option_d: "5",
    correct_option: "B",
    explanation: "Money laundering involves three steps: Placement, Layering, and Integration."
  },
  {
    ...
    correct_option: "D",
    explanation: "Verification is not part of the money laundering process."
  },
  {
...
questions.forEach((q, index) => {
  ...
  if ((index + 1) % 10 === 0) {
    console.log(`  Inserted ${index + 1} questions...`);
  }
});

console.log(`✅ Inserted all ${questions.length} questions`);

// Insert sample lecture slides
console.log('\nAdding sample lecture slides...');
const slides = [
  {
    title: 'Introduction to AML/CFT Compliance',
    description: 'Overview of Anti-Money Laundering and Counter Financing of Terrorism regulations',
    file_name: 'AML_CFT_Training.pdf',
    file_path: '/materials/AML_CFT_Training.pdf',
    file_size: '2.4 MB',
    category: 'Compliance',
    icon: '📋',
    has_material: 1
  ...
  {
    title: 'KYC Procedures and Best Practices',
    description: 'Know Your Customer verification processes and documentation requirements',
    file_name: 'Suspicious_Account_Handling_SOP.pdf',
    file_path: '/materials/Suspicious_Account_Handling_SOP.pdf',
    file_size: '3.1 MB',
    category: 'Compliance',
    icon: '🔍',
    has_material: 1
  },
  {
    ...
    description: 'How to identify and report suspicious transactions effectively (Material under development)',
    file_name: 'Transaction_Monitoring.pdf',
    file_path: '/materials/Transaction_Monitoring.pdf',
    file_size: '2.8 MB',
    category: 'Compliance',
    icon: '📊',
    has_material: 0
  ...
  {
    title: 'PSP Regulations in Bangladesh',
    description: 'Understanding Payment Service Provider licensing and compliance requirements',
    file_name: 'emoney-regulations.pdf',
    file_path: '/materials/emoney-regulations.pdf',
    file_size: '4.2 MB',
    category: 'Regulatory',
    icon: '⚖️',
    has_material: 1
...
const insertSlide = db.prepare(`
  INSERT INTO lecture_slides (title, description, file_name, file_path, file_size, category, icon, has_material)
  VALUES (?, ?, ?, ?, ?, ?, ?, ?)
`);
...
console.log(`✅ Added ${slides.length} sample lecture slides`);
...
const stats = {
  ...
  tests: db.prepare('SELECT COUNT(*) as count FROM tests').get(),
  questions: db.prepare('SELECT COUNT(*) as count FROM questions').get(),
};

console.log(`Users: ${stats.users.count}`);
console.log(`Tests: ${stats.tests.count}`);
console.log(`Questions: ${stats.questions.count}`);
